...
For Gravity UI use [UIKitTreeSelect](/docs/lab-uikit-uikittreeselect--docs).

## Basic usage

```tsx
import {useState} from 'react';
import {TreeSelect} from '@gravity-ui/tree-select';
...
To fix our example we need to use `getItemId` prop and explicitly tell `TreeSelect` to use items values as uniq ids:
